...
// Heun-Euler RK1(2) DDE solver
SolverResults heun_euler_dde_solver(const SolverParameters& Params)
{
    ...
    if (!Params.error_estimate)
    {
        while (t < t1)
        {
            t_delayed = t - Params.delay_times[0];
    ...
    else if (!Params.variable_steps)
    {
        ...
        while (t < t1)
        {
            ...
            y_delayed = interpolate_delay(t_delayed, Times, result, Params.t0, Params.history_function);
            k2 = deriv(t + dt, y_heun, y_delayed);
    ...
    else
    {
        while (t < t1)
        {
            ...
            while (!step_accepted)
            {
                ...
                y_delayed = interpolate_delay(t_delayed, Times, result, Params.t0, Params.history_function);
                k2 = deriv(t + current_dt, y_temp, y_delayed);
...
// Bogacki-Shampine RK2(3) DDE solver
SolverResults bogacki_shampine_dde_solver(const SolverParameters& Params)
{
    auto deriv = Params.delay_derivative;
    auto y     = Params.initial_conditions;
    auto t     = Params.t0;
    ...
    if (!Params.error_estimate)
    {
        ...
        while (t < t1)
        {
            ...
            y_delayed = interpolate_delay(t_delayed, Times, result, Params.t0, Params.history_function);
            k2 = deriv(t + dt_half, y_temp, y_delayed);
    ...
    else if (!Params.variable_steps)
    {
        ...
        while (t < t1)
        {
            ...
            for (size_t i = 0; i < y_temp.size(); ++i)
                y_temp[i] += dt_34 * k2[i];
            t_delayed = t + dt_34 - Params.delay_times[0];
            y_delayed = interpolate_delay(t_delayed, Times, result, Params.t0, Params.history_function);
            k3 = deriv(t + dt_34, y_temp, y_delayed);

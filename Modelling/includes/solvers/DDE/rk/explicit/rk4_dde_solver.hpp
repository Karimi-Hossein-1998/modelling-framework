...
// DDE version of Runge-Kutta fourth-order method (RK4)
// Solves delay differential equations of the form:
// dy/dt = f(t, y(t), y(t - tau))
// where tau is the delay time
// Uses RK4 method: y_{n+1} = y_n + dt/6 * (k1 + 2*k2 + 2*k3 + k4)
inline SolverResults rk4_dde_solver(const SolverParameters& Params)
{
    ...
    if (!f)
    {
        throw std::invalid_argument("[rk4_dde_solver] DDE derivative function is required");
    }
    if (delay_times.empty())
    {
        throw std::invalid_argument("[rk4_dde_solver] At least one delay time is required for DDE");
    }
    ...
    for (size_t i = 0; i < num_steps; ++i)
    {
        ...
        // RK4 step 8: Update solution using k1, k2, k3, k4: y_{n+1} = y_n + dt/6 * (k1 + 2*k2 + 2*k3 + k4)
        for (size_t j = 0; j < N; ++j)
        {
...
// Basic interface wrapper for rk4_dde_solver
inline Matrix rk4_dde_solver(
    DelayFunc    deriv,
    const dVec& y0,
    double       t0,
    double       t1,
...
)
{
    ...
    params.is_dde            = true;
    return rk4_dde_solver(params).solution;
} 
...
// General DDE version of Kutta's third-order method (RK3) for multiple delays
...
inline SolverResults rk3_dde_general_solver(const SolverParameters& Params)
{
    ...
    if (!f)
    {
        throw std::invalid_argument("[rk3_dde_general_solver] DDE derivative function is required");
    }
    if (delay_times.empty())
    {
        throw std::invalid_argument("[rk3_dde_general_solver] At least one delay time is required for DDE");
    }
    ...
    for (size_t i = 0; i < num_steps; ++i)
    {
        ...
        k3 = f(t + dt, y_temp, y_delayed_states);
        for (size_t j = 0; j < N; ++j)
...
// Basic interface wrapper for rk3_dde_general_solver
inline Matrix rk3_dde_general_solver(
...
)
{
    ...
    params.is_dde                  = true;
    return rk3_dde_general_solver(params).solution;
} 
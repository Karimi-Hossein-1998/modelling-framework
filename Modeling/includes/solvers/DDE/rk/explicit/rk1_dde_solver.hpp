...
// DDE version of Euler's method (RK1)
// Solves delay differential equations of the form:
// dy/dt = f(t, y(t), y(t - tau))
// where tau is the delay time
inline SolverResults rk1_dde_solver(const SolverParameters& Params)
{
    ...
    if (!f)
    {
        throw std::invalid_argument("[rk1_dde_solver] DDE derivative function is required");
    }
    if (delay_times.empty())
    {
        throw std::invalid_argument("[rk1_dde_solver] At least one delay time is required for DDE");
    }
    ...
    for (size_t i = 0; i < num_steps; ++i)
    {
        ...
        // Euler step: y_{n+1} = y_n + dt * f(t_n, y_n, y_delayed)
...
// Basic interface wrapper for rk1_dde_solver
inline Matrix rk1_dde_solver(
    DelayFunc    deriv,
    const dVec&  y0,
    double       t0,
    double       t1,
...
)
{
    ...
    params.is_dde            = true;
    return rk1_dde_solver(params).solution;
}
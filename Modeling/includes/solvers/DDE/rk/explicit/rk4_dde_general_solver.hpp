...
// General DDE version of Runge-Kutta fourth-order method (RK4) for multiple delays
...
inline SolverResults rk4_dde_general_solver(const SolverParameters& Params)
{
    ...
    if (!f)
    {
        throw std::invalid_argument("[rk4_dde_general_solver] DDE derivative function is required");
    }
    if (delay_times.empty())
    {
        throw std::invalid_argument("[rk4_dde_general_solver] At least one delay time is required for DDE");
    }
...
// Basic interface wrapper for rk4_dde_general_solver
inline Matrix rk4_dde_general_solver(
...
)
{
    ...
    params.is_dde                  = true;
    return rk4_dde_general_solver(params).solution;
} 
...
// General DDE version of Midpoint method (RK2) for multiple delays
// Solves delay differential equations of the form:
// dy/dt = f(t, y(t), y(t - τ₁), y(t - τ₂), ..., y(t - τₙ))
// where τ_i are the delay times
// Uses the midpoint method: y_{n+1} = y_n + dt * f(t_n + dt/2, y_n + dt/2 * k1, y_delayed_mid)
inline SolverResults rk2_dde_general_solver(const SolverParameters& Params)
{
    ...
    if (!f)
    {
        throw std::invalid_argument("[rk2_dde_general_solver] DDE derivative function is required");
    }
    if (delay_times.empty())
    {
        throw std::invalid_argument("[rk2_dde_general_solver] At least one delay time is required for DDE");
    }
...
// Basic interface wrapper for rk2_dde_general_solver
inline Matrix rk2_dde_general_solver(
...
)
{
    ...
    params.is_dde                  = true;
    return rk2_dde_general_solver(params).solution;
} 
...
// DDE version of Midpoint method (RK2)
// Solves delay differential equations of the form:
// dy/dt = f(t, y(t), y(t - tau))
// where tau is the delay time
// Uses the midpoint method: y_{n+1} = y_n + dt * f(t_n + dt/2, y_n + dt/2 * k1, y_delayed)
inline SolverResults rk2_dde_solver(const SolverParameters& Params)
{
    ...
    if (!f)
    {
        throw std::invalid_argument("[rk2_dde_solver] DDE derivative function is required");
    }
    if (delay_times.empty())
    {
        throw std::invalid_argument("[rk2_dde_solver] At least one delay time is required for DDE");
    }
...
// Basic interface wrapper for rk2_dde_solver
inline Matrix rk2_dde_solver(
    DelayFunc    deriv,
    const dVec& y0,
    double       t0,
    double       t1,
...
)
{
    ...
    params.is_dde            = true;
    return rk2_dde_solver(params).solution;
} 